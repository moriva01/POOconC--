...
void colchon_ahorro(usuario &p)
{
    string trivia, respuesta;

    if (p.getColchon() != 0)
    {
        for (colchon &q : lista_colchon)
        {
            if (p.getNumeroDocumento() == q.getIdCliente())
            {
                cout << "saldo en colchon: " << q.getSaldo() << endl;
                int eleccion;
                cout << "que desea hacer?" << endl;
                cout << "1- meter plata al colchon" << endl
                     << "2- proteger colchon" << endl
                     << "3- retirar" << endl
                     << "4-salir" << endl;
                cin >> eleccion;
                ...
                case 1:
                    float cantidad;
                    cout << "Digite la cantidad de plata a pasar: " << endl;
                    cin >> cantidad;
                    ...
                    if (p.getSaldo() < cantidad)
                    {
                        cout << "El saldo no es suficiente para pasar al colchon" << endl;
                        break;
                    }
                ...
                case 2:
                    int eleccion2;
                    cout << "como desea proteger su colchon?" << endl;
                    cout << "1- por tiempo" << endl
                         << "2- solo un amigo puede desbloquear" << endl
                         << "3- pregunta trivia" << endl
                         << "4- salir" << endl;
                    cin >> eleccion2;
                    ...
                    case 1:
                        q.setProteccion(1);
                        q.setPregunta("bloqueo por tiempo");
                        int horas;
                        cout << "digite la cantidad de tiempo en horas que desea bloquear su colchon:" << endl;
                        cin >> horas;
                        q.setRespuesta(to_string(horas));
                        break;
                    case 2:
                        q.setProteccion(2);
                        q.setPregunta("bloqueo por amigo");
                        int long long numero;
                        cout << "digite el numero de su amigo para bloquear su colchon:" << endl;
                        cin >> numero;
                        q.setRespuesta(to_string(numero));
                        break;
                    case 3:
                        q.setProteccion(3);
                        cout << "cual sera su pregunta trivia?" << endl;
                        cin.ignore();         // Limpiar el búfer de entrada antes de leer la pregunta
                        getline(cin, trivia); // Leer la pregunta de la trivia
                        q.setPregunta(trivia);

                        cout << "cual sera la respuesta de la  trivia?" << endl;
                        getline(cin, respuesta); // Leer la respuesta de la trivia
                ...
                case 3:
                    switch (q.getProteccion())
                    {
                    case 1:
                        cout << "error, aun quedan " << q.getRespuesta() << " horas, antes de desbloquear su colchon" << endl;
                        break;

                    case 2:
                        cout << "error, solo su amigo con numero " << q.getRespuesta() << " puede desbloquear su colchon" << endl;
                        break;

                    case 3:
                        cout << "pregunta: " << q.getPregunta() << "? " << endl;
                        cin.ignore(); // Limpiar el búfer de entrada
                        cout << "digite su respuesta: " << endl;
                        cin >> respuesta;

                        if (respuesta == q.getRespuesta())
                        {
                            p.setSaldo(p.getSaldo() + q.getSaldo());
                        ...
                        {
                            cout << "error, esa no es la respuesta para la pregunta" << endl;
                        }
                        break;
                    default:
                        break;
    ...
    {
        colchon auxc;
        auxc.setIdCliente(p.getNumeroDocumento());
        auxc.setSaldo(0);
        auxc.setProteccion(0);
        auxc.setPregunta("");
        auxc.setRespuesta("");

        lista_colchon.push_back(auxc);
        p.setColchon(1);
    ...
    for (usuario &p : lista_usuario) // recorre la lista
    {
...

        {
            while (true) // si esta registrado muestra el menu del usuario
            {
                ...
                {
                case 1:
                    colchon_ahorro(p); // colchon ahorro
                    break;
                case 2:
